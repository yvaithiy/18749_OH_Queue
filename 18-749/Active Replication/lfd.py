	...
	while True:
		...
		if last_time and len(last_time) > 1:
			...
			if (abs(current_time-last_time) > THRESHOLD):
				alive = False
				print("Replica Died! (time threshold exceeded)")
			else:
				if alive == False:
					print("Replica Alive Again! (valid heatbeat)")
				alive = True
		else:
			noTimeFound += 1		
			if noTimeFound > 10:
				alive = False
				print("Replica Died! (time not found)")
			else:
				if alive == False:
					print("Replica Alive Again! (heartbeat found again)")
				alive = True	
