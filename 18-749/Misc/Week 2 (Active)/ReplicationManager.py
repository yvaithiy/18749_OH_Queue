...
def check_membership_change():
    ...
    if(len(ALIVE_IPs) == 2 && flag_all_replicas==False):
        flag_all_replicas = True
        ALIVE_IPs_ORIGINAL.append([ALIVE_IPs[0],0])
        ALIVE_IPs_ORIGINAL.append([0, ALIVE_IPs[1]])
        ALIVE_IPs_ORIGINAL.append(ALIVE_IPs.copy())
        print ALIVE_IPs_ORIGINAL
    update_alive_replica()
...
def pack_message():
    ...
    if(flag_all_replicas == True):
        if(len(ALIVE_IPs)==1):
            if(ALIVE_IPs[0] == ALIVE_IPs_ORIGINAL[0][0]):
                for IP in ALIVE_IPs_ORIGINAL[0]:
                    message = message + IP + " " 
            elif(ALIVE_IPs[0] == ALIVE_IPs_ORIGINAL[1][1]):
                for IP in ALIVE_IPs_ORIGINAL[1]:
                    message = message + IP + " " 
        else:
            for IP in ALIVE_IPs_ORIGINAL[2]:
                    message = message + IP + " " 
